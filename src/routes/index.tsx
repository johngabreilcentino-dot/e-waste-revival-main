...
import { ArrowRight, Recycle, MapPin, ScrollText, Sparkles, Cpu, Leaf, ShieldCheck } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Discover e-waste collection drives near you and learn about Philippine recycling policies." },
    ],
  }),
  component: HomePage,
});
    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20 lg:py-28 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full border border-border bg-card/70 px-4 py-1.5 text-xs font-medium text-primary backdrop-blur">
              <Sparkles className="h-3.5 w-3.5" /> Greener electronics for the Philippines
            </span>
            ...
            <p className="mt-6 text-lg text-muted-foreground max-w-xl leading-relaxed">
              ...
              nationwide movement to keep electronics out of landfills.
            </p>
            ...
            <div className="rounded-[2rem] bg-card border border-border p-8 shadow-elevated">
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  { icon: Recycle, t: "Batteries", c: "accent" },
              ...
              <div className="mt-6 rounded-2xl bg-primary text-primary-foreground p-5">
                ...
                <div className="mt-1 font-display text-xl">Wipe your data before recycling devices.</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="explore" className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-8">
          <div>
            <span className="text-xs font-semibold uppercase tracking-wider text-primary">Explore the map</span>
            <h2 className="mt-2 text-4xl sm:text-5xl font-semibold text-foreground">
              Drives & policies, on one map.
            </h2>
            <p className="mt-3 text-muted-foreground max-w-2xl">
              Toggle between upcoming collection events and regional regulations across the Philippines.
            </p>
          </div>
          <div className="inline-flex rounded-full border border-border bg-card p-1 shadow-soft">
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-medium transition ${
                tab === "drives" ? "bg-primary text-primary-foreground shadow-soft" : "text-foreground/70 hover:text-foreground"
              }`}
            >
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-medium transition ${
                tab === "policies" ? "bg-primary text-primary-foreground shadow-soft" : "text-foreground/70 hover:text-foreground"
              }`}
            >
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center max-w-2xl mx-auto">
          <span className="text-xs font-semibold uppercase tracking-wider text-primary">Why it matters</span>
          <h2 className="mt-2 text-4xl sm:text-5xl font-semibold text-foreground">
        ...
        <div className="mt-12 grid md:grid-cols-3 gap-6">
          {[
            ...
            { t: "Comply with RA 11898", d: "The EPR Act now requires producers and citizens to manage e-waste responsibly.", icon: ShieldCheck },
          ].map(({ t, d, icon: Icon }) => (
            <div key={t} className="rounded-3xl bg-card border border-border p-8 hover:shadow-elevated transition-shadow">
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-12">
        <div className="rounded-[2rem] p-12 sm:p-16 text-primary-foreground relative overflow-hidden" style={{ background: "var(--gradient-hero)" }}>
          <div className="relative z-10 max-w-2xl">
            <h2 className="text-4xl sm:text-5xl font-semibold">Got old electronics gathering dust?</h2>
            <p className="mt-4 text-primary-foreground/85 text-lg">
              Find your nearest drive or get in touch — we'll help you dispose of them the right way.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/contact" className="inline-flex items-center gap-2 rounded-full bg-cream text-primary px-6 py-3 font-semibold hover:opacity-90 transition">
                Contact us <ArrowRight className="h-4 w-4" />
              </Link>
              <Link to="/services" className="inline-flex items-center gap-2 rounded-full border border-primary-foreground/30 px-6 py-3 font-semibold hover:bg-primary-foreground/10 transition">
                See services
              </Link>
            </div>
          </div>
